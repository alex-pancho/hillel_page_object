...
def test_add_new_car(garage_page):
    garage_page.add_new_car("BMW", 123)
    element = garage_page.item("new_car")
    assert element.is_visible(), f"Not found: {element._locator}"

def test_add_car_button_visibility(garage_page):
    element = garage_page.item("add_car")
    assert element.is_visible(), f"Add Car button is not visible: {element._locator}"
